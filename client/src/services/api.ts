...
export const proClubService = {
  processData(data: any): { matches: Match[]; players: Player[]; stats: ClubStats } {
    ...
    const stats: ClubStats = {
      ...
      currentStreak: {
        type: this.parseStreakType(overallStats?.currentStreak),
        count: this.parseStreakCount(overallStats?.currentStreak)
      },
      bestStreak: 0 // Backend não calcula isso no momento
    };
