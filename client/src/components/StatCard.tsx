...
export function StatCard({ icon: Icon, label, value, delay = 0 }: StatCardProps) {
  ...
  return (
    <Reveal delay={delay}>
      <motion.div 
        whileHover={{ y: -8 }}
        className="glass-dark card-hover rounded-2xl p-6 h-full flex flex-col justify-between relative overflow-hidden group"
      >
        ...
        <div className="relative z-10">
          <div className="text-4xl md:text-5xl font-bold text-white mb-2 tracking-tighter flex items-baseline">
            {isNumber || !isNaN(numericValue) ? (
              <>
                {displayValue.toFixed(numericValue % 1 === 0 ? 0 : 1)}
                <span className="text-2xl ml-1 opacity-50">{suffix}</span>
              </>
            ) : (
              value
            )}
