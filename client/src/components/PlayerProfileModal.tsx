...
import { X, Target, Zap, Shield, TrendingUp, Share2 } from 'lucide-react';
import { getPositionLabel, getPositionColor } from '@/lib/playerUtils';
import { PlayerAttributesRadar } from './PlayerAttributesRadar';
...
export const PlayerProfileModal = React.memo(({ player, isOpen, onClose, allPlayers = [] }: PlayerProfileModalProps) => {
          ...
          >
            <div className="pointer-events-auto w-full max-w-2xl glass-dark rounded-3xl border border-white/20 overflow-hidden max-h-[90vh] overflow-y-auto">
              ...
              <div className="p-8 space-y-8">
                ...
                {allPlayers.length > 0 && (
                  <div>
                    <h3 className="text-sm font-bold text-white/60 uppercase tracking-widest mb-4">Análise de Atributos</h3>
                    <div className="glass-dark p-6 rounded-xl border border-white/10">
                      <PlayerAttributesRadar player={player} allPlayers={allPlayers} />
                    </div>
                  </div>
                )}
              </div>
              ...
              {/* Footer */}
              <div className="px-8 py-4 border-t border-white/10 flex justify-between gap-3">
                <button
                  onClick={() => setIsShareOpen(true)}
                  className="px-6 py-2 rounded-lg bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold hover:opacity-90 transition-opacity flex items-center gap-2"
                >
                  <Share2 size={18} />
                  Gerar Card
                </button>
